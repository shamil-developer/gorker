...
retry, panic recovery, структурированные логи и graceful shutdown.
...
`Start` сначала валидирует параметры. При ошибке он синхронно возвращает
`(nil, error)` и не создаёт горутину. После успешного запуска возвращается
`(Result, nil)`, где `Result` — канал `<-chan error`. При успешном завершении
канал закрывается без значения. При окончательной runtime-ошибке он возвращает
одну ошибку и закрывается.
...
`MaxAttempts` включает первый запуск. Значения `0` и `1` означают один запуск
без повторов. При включённом retry значение должно быть не меньше двух. Timeout
...
безопасна для конкурентного использования. Logger передаётся исполнителю и
режиму запуска; они напрямую пишут сообщения в месте выполнения операции.
